...
import { ChipRiesgo } from '@/components/ui';
import type { Capricho, Gasto, Ingreso, NivelRiesgo } from '@/lib/types';
...
export default function PanelPage() {
  ...
  const caprichosPendientes = db
    .prepare("SELECT * FROM caprichos WHERE estado = 'pendiente' ORDER BY fecha_deseada")
    .all() as Capricho[];

  // Ahorro del mes = recibido − pagado dentro del mes actual
  const mes = new Date().toISOString().slice(0, 7);
  const recibido = (db
    .prepare("SELECT COALESCE(SUM(monto),0) AS s FROM ingresos WHERE estado='recibido' AND fecha_prevista LIKE ?")
    .get(mes + '%') as { s: number }).s;
  const pagado = (db
    .prepare("SELECT COALESCE(SUM(monto),0) AS s FROM gastos WHERE estado='pagado' AND fecha_prevista LIKE ?")
    .get(mes + '%') as { s: number }).s;
  const ahorroMes = recibido - pagado;
  ...
  const proximosIngresos = (db
    .prepare("SELECT * FROM ingresos WHERE estado != 'recibido' ORDER BY fecha_prevista LIMIT 3")
    .all() as Ingreso[]);
  const proximosGastos = (db
    .prepare("SELECT * FROM gastos WHERE estado != 'pagado' ORDER BY fecha_prevista LIMIT 3")
    .all() as Gasto[]);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4">
        <div className="tarjeta col-span-2 bg-gradient-to-br from-barbie-500 to-barbie-400 !border-barbie-300 md:col-span-1">
        ...
        <div className="tarjeta">
          ...
          <p className="mt-1 truncate text-[11px] text-tinta/50">
            {caprichosPendientes[0] ? `${caprichosPendientes[0].nombre} · ${euros(caprichosPendientes[0].monto)}` : 'Nada en la lista de deseos'}
          </p>
        </div>
      ...
      <section className="tarjeta">
        <div className="mb-3 flex items-center justify-between">
          ...
          <Link href="/prevision" className="boton-suave">Ver día a día →</Link>
        </div>
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          {[
            ...
            <div key={plazo} className="rounded-2xl border border-barbie-100 bg-white/60 p-3 text-center">
        ...
        <div className="mt-4 grid grid-cols-2 gap-3">
          <div className="rounded-2xl bg-emerald-50/70 p-3">
            ...
            <p className="font-display text-lg font-bold text-ingreso">+{euros(prevision.totalIngresosPrevistos)}</p>
          </div>
          <div className="rounded-2xl bg-red-50/70 p-3">
            ...
            <p className="font-display text-lg font-bold text-gasto">−{euros(prevision.totalGastosPrevistos)}</p>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-2">
        {prevision.alertas.map((a) => (
          ...
          >
            <ChipRiesgo nivel={a.nivel} texto={a.nivel === 'verde' ? 'Todo bien' : a.nivel === 'naranja' ? 'Atención' : 'Alerta'} />
            <div>
      ...
      <section className="grid gap-4 md:grid-cols-2">
        <div className="tarjeta">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Link href="/ingresos" className="boton-suave">Todas →</Link>
          </div>
          {proximosIngresos.length === 0 && <p className="text-xs text-tinta/50">Nada previsto todavía.</p>}
          {proximosIngresos.map((i) => (
            <div key={i.id} className="flex items-center justify-between border-b border-barbie-50 py-2 text-sm last:border-0">
        ...
        <div className="tarjeta">
          <div className="mb-2 flex items-center justify-between">
            ...
            <Link href="/gastos" className="boton-suave">Todas →</Link>
          </div>
          {proximosGastos.length === 0 && <p className="text-xs text-tinta/50">Nada previsto todavía.</p>}
          {proximosGastos.map((g) => (
            <div key={g.id} className="flex items-center justify-between border-b border-barbie-50 py-2 text-sm last:border-0">
